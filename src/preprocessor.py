...
from .config import (
    DATA_DIR, PROCESSED_DIR, MEDIAPIPE_CONFIG,
...
class Preprocessor:
    ...
    def __init__(self):
        self.mp_hands = mp.solutions.hands.Hands(**MEDIAPIPE_CONFIG)
    ...
    def extract_features_from_video(self, video_path: str) -> np.ndarray:
        ...
        # NumPy 배열로 변환
        coords = np.array(coords_list)           # (frames, 42)
    ...
    def process_dataset(self, dataset_dir=None, output_dir=None):
        ...
        # NumPy 배열로 변환
        X = np.array(all_windows, dtype=np.float32)  # (N, 15, 127)
        ...
        print(f"  - Normalized coords: 42 (21 landmarks x 2)")
        print(f"  - Wrist angle: 1")
        print(f"  - Velocity: 42")
        print(f"  - Acceleration: 42")
